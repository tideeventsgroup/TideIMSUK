...
export function useIncidents(eventId: string | null) {
  ...
  useEffect(() => {
    ...
    async function loadInitial() {
      const { data } = await client.models.Incident.list({ filter: { eventId: { eq: id } } });
      if (!cancelled) {
    ...
    const createSub = client.models.Incident.onCreate({ filter: { eventId: { eq: id } } }).subscribe({
      next: (incident) => {
        setIncidents((prev) => [incident as unknown as Incident, ...prev.filter((i) => i.id !== incident.id)]);
      },
    });

    const updateSub = client.models.Incident.onUpdate({ filter: { eventId: { eq: id } } }).subscribe({
      next: (incident) => {
        setIncidents((prev) =>
